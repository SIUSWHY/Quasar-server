...
async function callsLogic(
  ...
  socket.on(
    'set_call_data',
    async (data: { userId: string; comUserId: string; timeOfStartCall: Date; timeOfEndCall: Date }) => {
      const call = await Call.create({
        userId: data.userId,
        comUserId: data.comUserId,
        timeOfEndCall: data.timeOfEndCall,
        timeOfStartCall: data.timeOfStartCall,
      });
